...
# ref: https://gist.github.com/ahmetb/7ce6d741bd5baa194a3fac6b1fec8bb7
...
main(){
  REPOSITORY="${1}"
  DATE="${2}"
  for img in $(gcloud container images list --repository=${REPOSITORY}); do
    local C=0
    for digest in $(gcloud container images list-tags ${img} --limit=999999 --sort-by=TIMESTAMP \
    ...
    echo "Deleted ${C} images in ${img}." >&2
  done
}
